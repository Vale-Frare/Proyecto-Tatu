    ...
    constructor(x1: number, y1: number, x2: number, y2: number, cantidad_bolitas: number, scale: number, scene: Phaser.Scene) {
        ...
        for (let i = 0; i < cantidad_bolitas + 1; i++) {
            if (i != 0) {
                ...
                //var body = scene.matter.add.circle(Phaser.Math.Linear(x1, x2, i/cantidad_bolitas), Phaser.Math.Linear(y1, y2, i/cantidad_bolitas), scale);
                scene.matter.add.gameObject(_, {isSensor: true, shape: 'circle'});
                //_.setStatic(true);
                _.setDepth(20);
                this.bolitas.push(_);
            }else {
